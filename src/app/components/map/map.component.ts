...
export class MapComponent implements OnInit {
  ...
  countriesInfo$: Observable<Country[]>;

  constructor(private zone: NgZone, private mainService: MainService) { }

  ngOnInit(): void {

    this.countriesInfo$ = this.mainService.getAllCountriesStats();
    this.countriesInfo$.subscribe(res => console.log(res));
  }
  ...
  this.zone.runOutsideAngular(() => {
    ...
    let mapData = [
      { "id":"AF", "name":"Afghanistan", "value":0, "color": chart.colors.getIndex(0) },
    ...
    this.countriesInfo$.subscribe(res => {
      let apiIndex = res.length-1;
      let mapDataIndex = 0;
      console.log(mapData[mapDataIndex].name);
      console.log(res[apiIndex].country);
      console.log(mapData[mapDataIndex].value);
      console.log(res[apiIndex].cases);
      mapData[mapDataIndex]["value"] = res[apiIndex].cases;


      // while(mapDataIndex < mapData.length && apiIndex >= 0){
      //   if(mapData[mapDataIndex].name === res[apiIndex].country ){
      //     mapData[mapDataIndex].value = res[apiIndex].cases;
      //     ++mapDataIndex;
      //     --apiIndex;
      //   }else{
